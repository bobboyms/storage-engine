...
// RecordID é o identificador externo de um registro no heap v2.
...
//   bits 62-16: PageID (47 bits — max 2^47 páginas = 1 exabyte com 8KB)
//   bits 15-0:  SlotID (16 bits — max 65535 slots por página)
//
// Valores especiais:
//   - NoRecordID (-1): sentinela "sem versão anterior" (compatível com v1)
//   - 0: inválido (PageID 0 é reservado pelo PageFile)
const (
	slotIDBits uint64 = 16
	slotIDMask uint64 = (1 << slotIDBits) - 1

	// MaxSlotID é o maior SlotID possível. Páginas de 8KB raramente
	// chegam perto disso — são limitadas pelo espaço, não pelo número.
	MaxSlotID uint16 = uint16(slotIDMask)

	// MaxPageID é o maior PageID possível. 2^47 - 1 páginas de 8KB
...
// EncodeRecordID. Não valida — passar NoRecordID ou 0 produz resultados
