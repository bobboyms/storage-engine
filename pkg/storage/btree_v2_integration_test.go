...
// newCipherFor gera um AES-GCM com key aleatória pra usar em testes.
...
// index BTreeFormatV2 funciona drop-in no engine. O engine usa
...
// WAL já populado must auto-sincronizar o lsnTracker. Sem isso, snapshots
// novos (SnapshotLSN=0) not enxergam records persistidos (CreateLSN>=1).
//
...
// é do engine, not específico do v2.
func TestEngine_AutoScanWALForMaxLSN(t *testing.T) {
	...
	// Sessão 2: reabre — auto-scan must advance lsnTracker para 5
	...
	for i := int64(1); i <= 5; i++ {
		...
		if !found {
			t.Fatalf("key %d invisible after reopen (LSN tracker not sincronizou)", i)
		}
		want := fmt.Sprintf(`{"id":%d}`, i)
		if doc != want {
			t.Fatalf("key %d: expected %q, got %q", i, want, doc)
		}
	}
}

// TestBTreeV2_Integration_Varchar prova que um index de VARCHAR em v2
...
// TestBTreeV2_Integration_ReopenWithTDE prova que o index persiste
// com cifra através do ciclo fechar/reopen — exatamente o buraco
...
func TestBTreeV2_Integration_ReopenWithTDE(t *testing.T) {
	...
	// Cria tabela com index v2 cifrado
	hm, _ := storage.NewHeapForTable(storage.HeapFormatV2, heapPath)
	...
	// Reabre com a MESMA key — dados voltam
	hm2, _ := storage.NewHeapForTable(storage.HeapFormatV2, heapPath)
	...
	// Not precisamos mais de Recover manual — NewStorageEngine agora faz
	...
	// Recover pode gerar nova versão via WAL replay; o doc must ser o mesmo
	if doc != `{"id":42,"payload":"confidencial"}` {
		t.Fatalf("doc corrupted: %q", doc)
	}
}
