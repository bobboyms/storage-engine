...
- **DDL + persistence** — `CREATE TABLE [IF NOT EXISTS] name (col TYPE [PRIMARY KEY] [INDEX], …)`. Column types map common SQL names (`INT`, `VARCHAR`, `BOOL`, `FLOAT`, `DATE`, …) to engine types; the schema (columns, types, indexes) is written to `schema.json` and restored on reopen, so tables and data survive restarts with no Go setup code. Without `IF NOT EXISTS`, re-creating an existing table name returns a duplicate-table error; with it, the statement is a silent no-op (the existing table and its data are left untouched).
- **Schema evolution** — `ALTER TABLE name ADD [COLUMN] col TYPE [INDEX]` and `ALTER TABLE name DROP [COLUMN] col`. A newly added column reads `NULL` on existing rows and is populated by later `INSERT`/`UPDATE`; adding it with `INDEX` builds an empty sidecar B+ tree that fills as those writes arrive (no backfill scan), so you can immediately query by the new field. `DROP COLUMN` removes the column and any index on it (the primary key column cannot be dropped). Changes are persisted to `schema.json` and survive reopen.
