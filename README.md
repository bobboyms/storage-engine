...
- [Production Guide](docs/ProductionGuide.md) — detailed implemented/partial/not-implemented feature matrix, failure model, and "when not to use" notes.
- [ADR 001: Page Format](docs/adr/001-page-format.md) — 8KB page layout, checksum, TDE, and format rationale.
- [Page-Based Migration Plan](docs/page_based_migration_plan.md) — historical implementation plan.
- [Relational Database Roadmap](docs/relational_database_roadmap.md) — broader roadmap notes.
- [Tutorial](docs/tutorial.md) — walkthrough material.

## Roadmap

High priority:

- evolve logical undo + CLRs into full ARIES with a persisted dirty-page table and page-oriented physical undo;
- `Serializable` isolation via range/predicate locking, plus write-skew detection;
- anti-starvation / fairness for readers and writers, and lock-contention metrics;
- persist and harden the free-space map; add a persistent free-page list and page allocator;
- explicit rejection of unknown on-disk format versions in `PageFile` and `WALReader`;
- structured metrics for WAL, BufferPool, recovery, vacuum, locks, and fsync.

Medium priority:

- overflow pages / TOAST for records larger than one page;
- configurable page/record compression;
- bloat-threshold-driven vacuum and offline table compaction / physical file shrink (`VACUUM FULL`) on top of the existing scheduled, activity-gated maintenance;
- background writer and read-ahead for sequential scans;
- native fuzzing (WAL, page files, slotted pages, B+ tree) and differential testing against a reference;
- full-engine benchmarks with large datasets and p95/p99 latency.

Lower priority / future:

- replication and failover; sharding/partitioning; hash index; LSM-tree; column-store.
...
- follow TDD (Red → Green → Refactor) and keep total coverage at or above 70%;
- run focused tests for the touched package, then `go test ./...`;
- run race/chaos/fault/stress suites when touching WAL, recovery, page files, transactions, concurrency, vacuum, or BufferPool;
- never change on-disk formats without versioning and migration/recovery notes;
- ensure `make lint` reports 0 issues and `make vuln` reports 0 affecting vulnerabilities;
- update [docs/ProductionGuide.md](docs/ProductionGuide.md) whenever guarantees or limitations change;
- keep everything in English.

## License

No license file is currently present. Add a `LICENSE` before publishing or accepting external contributions.
